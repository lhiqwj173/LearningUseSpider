...
async def Consumer(q2, url):
    ...
    while True:
        try:
            ...
            tag = url.split('/')[-1]
            typeMa = url.split('/')[3]
            dirName = f"javbus/{typeMa}/{dictTag[typeMa][tag]}"
            Web.mkDir(dirName)
...
async def Main(url):
    ...
    task2 = [asyncio.create_task(Producter2(q, q2)) for _ in range(72)]
    task3 = [asyncio.create_task(Consumer(q2, url)) for tmp in range(72)]
