...
async def fetch(url, session, data=False):
    ...
    try:
        ...
        print("(请确保SS/SSR代理已开启，并且端口为1080)fetch url error, maybe timeout or session closed!!!")
...
async def downloadImg(duilie, session, tag):
    if not os.path.exists(f'sehuatang/{tag}/{time.strftime("%Y%m%d", time.localtime())}'):
        os.makedirs(f'sehuatang/{tag}/{time.strftime("%Y%m%d", time.localtime())}')
    while True:
        #get也是异步操作，需await
...
if __name__ == '__main__':
    freeze_support()
    tagDict = {'fcppv':fcppv, 'allInOne':allInOne, 'anime':anime, 'chinese':chinese}
